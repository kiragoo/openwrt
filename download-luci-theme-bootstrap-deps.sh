...
# 从 OpenWrt 官方源下载 luci-theme-bootstrap 及其所有依赖包
# OpenWrt 24.10.0 x86_64

VERSION="24.10.0"
ARCH="x86_64"
BASE_URL="https://downloads.openwrt.org/releases/${VERSION}/packages/${ARCH}"
TARGET_DIR="luci-theme-bootstrap"

# luci-theme-bootstrap 包名
PACKAGE_NAME="luci-theme-bootstrap"
...
echo -e "${GREEN}=== 下载 luci-theme-bootstrap 及其所有依赖包 ===${NC}"
echo "版本: ${VERSION}"
echo "架构: ${ARCH}"
...
download_package() {
    ...
    # 尝试的 feed 顺序：首选 feed -> luci -> base -> packages
    ...
    if [ "$feed" != "packages" ] && [ -f "/tmp/Packages-packages.gz" ]; then
...
download_dependencies() {
    ...
    for dep in $deps; do
        ...
        # 尝试 luci feed
        if download_package "luci" "$dep"; then
            downloaded=true
            # 递归下载此依赖的依赖
        ...
        # 尝试 base feed
        if [ "$downloaded" = false ] && download_package "base" "$dep"; then
            downloaded=true
            download_dependencies "base" "$dep" "$visited_packages"
        fi
        
        # 尝试 packages feed
        if [ "$downloaded" = false ] && [ -f "/tmp/Packages-packages.gz" ] && download_package "packages" "$dep"; then
            downloaded=true
...
# 检查包是否存在
echo -e "${GREEN}查找 ${PACKAGE_NAME} 包...${NC}"
pkg_filename=$(get_package_filename "luci" "$PACKAGE_NAME")

if [ -z "$pkg_filename" ]; then
    echo -e "${RED}未找到 ${PACKAGE_NAME} 包${NC}"
    exit 1
fi

echo -e "${YELLOW}找到包: ${PACKAGE_NAME}${NC}"
echo -e "${YELLOW}文件名: ${pkg_filename}${NC}"
echo ""

# 下载主包
echo -e "${GREEN}下载 ${PACKAGE_NAME} 主包...${NC}"
if download_package "luci" "$PACKAGE_NAME"; then
    echo -e "${GREEN}✓ ${PACKAGE_NAME} 下载成功${NC}"
else
    echo -e "${RED}✗ ${PACKAGE_NAME} 下载失败${NC}"
    exit 1
fi

# 下载依赖
echo ""
echo -e "${GREEN}下载 ${PACKAGE_NAME} 的所有依赖...${NC}"
download_dependencies "luci" "$PACKAGE_NAME" ""
...
echo ""
echo "已下载的 luci-theme-bootstrap 相关包:"
ls -lh "${TARGET_DIR}"/luci-theme-bootstrap*.ipk 2>/dev/null | awk '{print "  " $9 " (" $5 ")"}' || echo "  (无)"
echo ""
echo "所有已下载的包列表:"
