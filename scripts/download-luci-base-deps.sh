...
# 从 OpenWrt 官方源下载 luci-base 及其所有依赖包
# 基于 openwrt.pkgs.org 的包信息结构
...
# luci-base 包信息
LUCI_BASE_PKG="luci-base"
LUCI_BASE_VERSION="25.340.26705~d88390b"
LUCI_BASE_FILENAME="luci-base_${LUCI_BASE_VERSION}_${ARCH}.ipk"
...
echo -e "${GREEN}=== 下载 luci-base 及其所有依赖包 ===${NC}"
...
# 下载 luci-base 主包
echo -e "${GREEN}下载 luci-base 主包...${NC}"
if download_package "luci" "$LUCI_BASE_PKG"; then
    echo -e "${GREEN}✓ luci-base 下载成功${NC}"
else
    echo -e "${RED}✗ luci-base 下载失败${NC}"
    exit 1
fi

# 下载 luci-base 的所有依赖
echo ""
echo -e "${GREEN}下载 luci-base 的所有依赖...${NC}"
download_dependencies "luci" "$LUCI_BASE_PKG" ""
